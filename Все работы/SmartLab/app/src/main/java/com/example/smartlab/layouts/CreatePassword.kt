...
fun CreatePassword(modifier: Modifier = Modifier,navController: NavController) {
    ...
    Column(modifier = modifier
        .fillMaxSize()
        .padding(top = 60.dp, start = 20.dp, end = 20.dp, bottom = 80.dp)){
        TextButton(text = "Пропустить",
            onClick = {navController.navigate("patientCard")},
            modifier = Modifier.align(Alignment.End), fontSize = 15.sp)
        ...
        Column(modifier =Modifier.align(Alignment.CenterHorizontally),
            ...
            Row(modifier=Modifier.align(Alignment.End)) {
                ...
                var pass by remember { mutableStateOf("") }
                    ...
                    pass = PassFieldValues.joinToString("")

                LaunchedEffect(pass) {
                    if (pass.length == 4) {
                        with(sharedPreferences.edit()){
                            putString("savedPwd",pass)
                            apply()
                        }
                        PassFieldValues.clear()
